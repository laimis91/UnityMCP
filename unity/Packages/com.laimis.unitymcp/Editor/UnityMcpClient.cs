...
#pragma warning restore CS0618
    ...
    private static string BuildSetSceneViewCameraResponse(JToken idToken, JObject root)
    {
        ...
        {
            throw new ArgumentException(
                "At least one Scene View camera setting must be provided: pivot, rotation, size, orthographic, or in2DMode.");
        }
        ...
        if (orthographic.HasValue)
        {
            sceneView.orthographic = orthographic.Value;
        }

        if (in2DMode.HasValue)
        {
            sceneView.in2DMode = in2DMode.Value;
        }

        sceneView.Repaint();

        var snapshot = BuildSceneViewCameraSnapshot(sceneView);
        return UnityMcpProtocol.CreateResult(idToken, new
        {
